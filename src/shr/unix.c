...
void unix_signal(struct sigaction *sig_stop, struct sigaction *sig_time)
{
	...
	sig_stop->sa_flags = 0;
	if ((sigemptyset(&sig_stop->sa_mask) == -1)
	    || (sigaction(SIGINT, sig_stop, NULL) != 0)) {
	...
	sig_time->sa_flags = 0;
	if ((sigemptyset(&sig_time->sa_mask) == -1)
	    || (sigaction(SIGALRM, sig_time, NULL) != 0)) {
...
void unix_environment(void)
{
	...
	snprintf(buffer, BUF_SIZE,
		 "Types: int: %i, long int: %i, size_t: %i, ssize_t: %i, time_t: %i",
		 sizeof(int), sizeof(long int), sizeof(size_t), sizeof(ssize_t),
		 sizeof(time_t));
#else
	snprintf(buffer, BUF_SIZE,
		 "Types: int: %lu, long int: %lu, size_t: %lu, ssize_t: %lu, time_t: %lu",
		 sizeof(int), sizeof(long int), sizeof(size_t), sizeof(ssize_t),
		 sizeof(time_t));
#endif
	log_simple(buffer);
