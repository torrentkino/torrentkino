...
LONG http_urlDecode(char *src, LONG srclen, char *dst, LONG dstlen)
{
	...
	if (src == NULL) {
		...
		info(_log, NULL,
		     "http_urlDecode(): Destination length is <= 0");
		return 0;
	} else if (srclen <= 0) {
		...
		if (*p1 == '+') {
			*p2 = ' ';
			...
			if (!sscanf(++p1, "%2x", &hex)) {
				...
				info(_log, NULL,
				     "http_urlDecode(): Broken characters");
				return 0;
			}
...
void http_404(TCP_NODE * n, char *keepalive)
{
	...
	resp_set_memory(r,
			...
			"%s"
			"\r\n"
			"%s", datebuf, LOG_NAME, size, keepalive, buffer);
}
	      ...
	      char *keepalive, const char *mimetype)
{
	...
	/* Compute answer */
	resp_set_memory(r, "HTTP/1.1 200 OK\r\n" "Date: %s\r\n" "Server: %s\r\n"
#ifdef __amd64__
			"Content-Length: %lu\r\n"
		     ...
		     const char *mimetype)
{
	...
	resp_set_memory(r_head,
			...
			"Last-Modified: %s\r\n" "%s" "\r\n", datebuf, LOG_NAME,
#ifdef __amd64__
			content_length, range_start, range_stop, filesize,
#else				/* __i386__ __arm__ */
			content_length,
			(unsigned int)range_start, (unsigned int)range_stop,
			filesize,
#endif
...
void http_206_boundary_head(RESPONSE * r_head, RESPONSE * r_file,
			    char *filename, size_t filesize,
			    const char *mimetype, char *boundary,
			    size_t * content_size)
{
	size_t range_start = r_file->data.file.f_offset;
	size_t range_stop = r_file->data.file.f_stop;
	...
	*content_size += resp_set_memory(r_head,
					 ...
					 (unsigned int)range_stop, filesize
#endif
	    );
}
