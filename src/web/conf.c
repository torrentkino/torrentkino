...
void conf_home_from_env(struct obj_conf *conf)
{
	if (getenv("HOME") == NULL || getuid() == 0) {
		...
		snprintf(conf->home, BUF_SIZE, "%s/%s", getenv("HOME"),
			 "Public");
	}
}
...
void conf_print(void)
{
	if (getenv("PWD") == NULL || getenv("HOME") == NULL) {
		info(_log, NULL,
		     "# Hint: Reading environment variables failed. sudo?");
		info(_log, NULL,
		     "# This is not a problem. But in some cases it might be useful");
		info(_log, NULL,
		     "# to use 'sudo -E' to export some variables like $HOME or $PWD.");
	}
