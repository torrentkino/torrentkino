...
{
	struct obj_mdb *mime =
	    (struct obj_mdb *)myalloc(sizeof(struct obj_mdb));
	mime->list = list_init();
	mime->hash = NULL;
...
{
	struct obj_mime *tuple =
	    (struct obj_mime *)myalloc(sizeof(struct obj_mime));
	...
	if (_main->mime->hash != NULL) {
		hash_put(_main->mime->hash, (UCHAR *) tuple->key,
			 strlen(tuple->key), tuple->val);
	}
...
const char *mime_find(char *filename)
{
	...
	/* Cached */
	if ((mime =
	     hash_get(_main->mime->hash, (UCHAR *) key, strlen(key))) != NULL) {
		return mime;
	}
#ifdef MAGIC
