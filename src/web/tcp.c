...
void tcp_start(void)
{
	...
	if (setsockopt
	    (_main->tcp->sockfd, SOL_SOCKET, SO_REUSEADDR, &optval,
	     sizeof(int)) == -1) {
	...
	if (setsockopt
	    (_main->tcp->sockfd, IPPROTO_IPV6, IPV6_V6ONLY, &optval,
	     sizeof(int)) == -1) {
	...
	if (bind
	    (_main->tcp->sockfd, (struct sockaddr *)&_main->tcp->s_addr,
	     _main->tcp->s_addrlen)) {
	...
	if (listen(_main->tcp->sockfd, listen_queue_length)) {
		...
		info(_log, NULL, "Listen queue length: %i",
		     listen_queue_length);
	}
...
void tcp_event(void)
{
	...
	if (epoll_ctl
	    (_main->tcp->epollfd, EPOLL_CTL_ADD, _main->tcp->sockfd,
	     &ev) == -1) {
...
void *tcp_thread(void *arg)
{
	...
	info(_log, NULL, "Thread[%i] - Max events: %i", id,
	     CONF_EPOLL_MAX_EVENTS);

	for (;;) {
		nfds =
		    epoll_wait(_main->tcp->epollfd, events,
...
void tcp_newconn(void)
{
	...
	for (;;) {
		...
		/* Accept */
		connfd =
		    accept(_main->tcp->sockfd, (struct sockaddr *)&c_addr,
			   &c_addrlen);
		if (connfd < 0) {
		...
		ev.data.ptr = listItem;
		if (epoll_ctl
		    (_main->tcp->epollfd, EPOLL_CTL_ADD, n->connfd,
		     &ev) == -1) {
			info(_log, NULL, strerror(errno));
