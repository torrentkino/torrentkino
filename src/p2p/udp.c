...
void udp_start(UDP * udp, int port, int multicast_mode)
{
	...
	/* Type of operation */
	udp->type =
	    (port == _main->conf->p2p_port) ? udp_p2p_worker : udp_dns_worker;
	...
	if (bind(udp->sockfd, (struct sockaddr *)&udp->s_addr, udp->s_addrlen)) {
...
void udp_event(UDP * udp)
{
	...
	if (epoll_ctl(udp->epollfd, EPOLL_CTL_ADD, udp->sockfd, &ev) == -1) {
...
void udp_multicast(UDP * udp, int mode, int runmode)
{
	...
	action =
	    (runmode == multicast_start) ? IPV6_JOIN_GROUP : IPV6_LEAVE_GROUP;
...
void udp_multicast(UDP * udp, int mode, int runmode)
{
	...
	action =
	    (runmode ==
	     multicast_start) ? IP_ADD_MEMBERSHIP : IP_DROP_MEMBERSHIP;
